...
class TestFormatTime(unittest.TestCase):
    ...
    def test_precision_seconds(self):
        self.assertEqual(cronbackoff._formatTime(
            10799, precision="seconds"), "2h 59m 59s")

    def test_precision_minutes(self):
        self.assertEqual(cronbackoff._formatTime(
            10799, precision="minutes"), "2h 59m")

    def test_precision_hours(self):
        self.assertEqual(cronbackoff._formatTime(
            10799, precision="hours"), "2h")
...
class TestStateMkDir(StateWrapper):
    ...
    def test_dir_wrong_owner(self):
        # It's non-trivial to create dir with wrong owner, so just use one that's
        # guaranteed to exist.
        self.state.dir = "/"
        with self.assertRaises(cronbackoff.CronBackoffException) as ctx:
            self.state._mkDir()
        self.assertTrue("not owned by" in str(ctx.exception))

    def test_file(self):
        self.state.dir = "/etc/fstab"
        with self.assertRaises(cronbackoff.CronBackoffException) as ctx:
            self.state._mkDir()
        self.assertTrue("not a dir" in str(ctx.exception))
    ...
    def test_symlink(self):
        ...
        with self.assertRaises(cronbackoff.CronBackoffException) as ctx:
            self.state._mkDir()
        self.assertTrue("symlink" in str(ctx.exception))
...
class TestStateRead(StateWrapper):
    ...
    def test_empty_state(self):
        ...
        with self.assertRaises(cronbackoff.CronBackoffException) as ctx:
            self.state._read()
        self.assertTrue("not a valid" in str(ctx.exception))
        os.unlink(self.state.filePath)
    ...
    def test_read_error(self):
        with open(self.state.filePath, 'w') as self.state.file:
            with self.assertRaises(cronbackoff.CronBackoffException) as ctx:
                self.state._read()
        self.assertTrue("not readable" in str(ctx.exception))
        os.unlink(self.state.filePath)
...
class TestStateSave(StateWrapper):
    ...
    def test_write_error(self):
        ...
        self.assertTrue("not open for" in str(ctx.exception))
